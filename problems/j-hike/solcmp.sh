...
for i in {0..49}; do
	./${title} <test/hike.$i.in >test/$i.ans
	./${title}2 <test/hike.$i.in >test/$i.ans2

    ./checker test/hike.$i.in test/sol1.$i. < test/$i.ans
    ./checker test/hike.$i.in test/sol2.$i. < test/$i.ans2
